...
format:
  # html:
    # page-layout: full
    # theme: cosmo        # any Bootstrap theme you like
    # df-print: paged
  pdf:
    pdf-engine: lualatex
execute:
...
ggplot(num_long, aes(value)) +
  ...
  labs(
    x = NULL, y = "Count"
  ) + theme_minimal() + latex_font_theme()
```
...
p_age + latex_font_theme(); p_tenure + latex_font_theme(); p_monthly + latex_font_theme(); p_total + latex_font_theme()
```
...
ggcorrplot(cor_mat,
           ...
           outline.color = "gray50") + latex_font_theme()
```
