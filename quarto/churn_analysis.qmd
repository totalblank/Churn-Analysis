...
ggplot(cat_long_churn, aes(level, fill = Churn)) +
  ...
  theme(axis.text.x = element_text(angle = 20, hjust = 0.5, vjust = 0.05))
```
...
df_use_2 |> 
  ...
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  labs(title = "Churn Rate by Risk Cohorts")
```
...
mi_tbl %>%
  ...
  coord_flip() +
  labs(title = "Mutual Information with Churn",
       x = "Feature", y = "Information Gain (MI)")
```
